...
public class DiscordBridgeAIO extends JavaPlugin {
    ...
    public void initHytaleService(AIOHytaleService service) {
        var papiPlugin = PluginManager.get().getPlugin(PluginIdentifier.fromString("HelpChat:PlaceholderAPI"));
        if(papiPlugin != null) {
            PlaceholderApiCompat.registerProcessors(service, (JavaPlugin) papiPlugin);
        }
    }

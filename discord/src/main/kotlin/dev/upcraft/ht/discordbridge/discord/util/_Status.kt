...
fun PresenceBuilder.fromServerStatus(serverStatus: ServerStatus = Hytale.serverStatus) {
    val (newStatus, newState) = when (serverStatus) {
        ServerStatus.RUNNING -> PresenceStatus.Online to Translations.BotStatus.online.translateNamed(replacements = buildMap { addStatusContext() })
        ServerStatus.STARTING -> PresenceStatus.Idle to Translations.BotStatus.starting.translateNamed(replacements = buildMap { addStatusContext() })
